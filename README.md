# GUI Furniture Stock and Sales Management Application

<div style="background-color:rgba(255, 203, 61, 10);">
  
> ## Notice
> This application is in the alpha state and so you may expect large amount of bugs <br>
> This repository does not contain the MSSQL database and therefore you must create it yourself (based off of the /Entities folder)

</div>

<a> This is a repo that stores our source code for the furniture selling program for a school group project. <br>
The school project has since been finished and now it sits abandoned (but unarchived) until further use. </a>

<a> This is the source code for the application. </a>
<a> If you wanted this application to work, please clone the repository and open it as a NetBeans project (or an IDE of your choice) and run it there. </a>
